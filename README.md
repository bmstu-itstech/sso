...
SSO — микросервис централизованной **аутентификации** и **авторизации** для ITS TECH приложений. Он выдаёт и валидирует **JWT**, хранит пользователей в **PostgreSQL**, ускоряет горячие операции через **in-memory cache**, и предоставляет **два транспорта** для интеграции:
